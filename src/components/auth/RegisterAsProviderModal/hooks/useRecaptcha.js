...
export const useRecaptcha = (open) => {
    ...
    const generateRecaptcha = useCallback(() => {
        // Always recreate or use existing if properly initialized?
        // Original logic checked if (!window.recaptchaVerifier).
        // Let's stick to original logic but ensure it works.
        // In provider modal (original code), it called clearRecaptcha() then new RecaptchaVerifier().
        // LoginModal useRecaptcha does if (!window.recaptchaVerifier).
